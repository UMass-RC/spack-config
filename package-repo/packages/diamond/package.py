...
from spack.pkg.builtin.diamond import Diamond as DiamondBase
...
class Diamond(DiamondBase):
    ...
    def cmake_args(self):
        return [
            f"-DBLAST_INCLUDE_DIR={self.spec['blast-plus'].prefix.include}/ncbi-tools++",
            f"-DBLAST_LIB_DIR={self.spec['blast-plus'].prefix.lib}",
            f"-DZSTD_LIBRARY={self.spec['zstd'].prefix}/lib/libzstd.so",
        ]
